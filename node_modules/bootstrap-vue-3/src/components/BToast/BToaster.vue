<template>
  <div :class="[positionClass]" class="b-toaster position-fixed p-3" style="z-index: 11">
    <b-toast v-for="toast in instance?.toasts(position).value" :id="toast.options.id" :key="toast.options.id"
      v-model="toast.options.value" :auto-hide="toast.options.autoHide" :delay="toast.options.delay"
      :no-close-button="toast.options.noCloseButton" :title="toast.content.title" :body="toast.content.body"
      :component="toast.content.body" :variant="toast.options.variant" @destroyed="handleDestroy" />
  </div>
</template>
...
import type { ToastInstance } from './plugin'
